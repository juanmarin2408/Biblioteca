...
class LibrosDao(ILibros):
        ...
        if conn:
            try:
                with conn.cursor() as cursor:
                    ...
                    cursor.execute(sql, (l.referencia, l.nombre, l.autor, l.año,
                                         l.genero, l.estado, l.fecha_inicio, l.fecha_fin))
                conn.commit()
                return True
            except Exception as e:
                pass  # Manejo silencioso de errores para GUI
    ...
    def modificar_libro(self, l: Libros) -> bool:
        ...
        if conn:
            try:
                with conn.cursor() as cursor:
                    sql = """UPDATE libros SET nombre=%s, autor=%s, anio=%s, genero=%s, estado=%s, 
                             fecha_inicio=%s, fecha_final=%s WHERE referencia=%s"""
                    cursor.execute(sql, (l.nombre, l.autor, l.año, l.genero,
                                         l.estado, l.fecha_inicio, l.fecha_fin, l.referencia))
                conn.commit()
                return True
            except Exception as e:
                pass  # Manejo silencioso de errores para GUI
